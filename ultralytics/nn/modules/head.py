...
from .block import DFL, SAVPE, BNContrastiveHead, ContrastiveHead, Proto, Residual, SWBlock, SwiGLUFFN
...
class Detect(nn.Module):
    ...
    def __init__(self, nc: int = 80, ch: Tuple = ()):
        ...
        self.cv2 = nn.ModuleList(nn.Sequential(SWBlock(x, c2), nn.Conv2d(c2, 4 * self.reg_max, 1)) for x in ch)
        self.cv3 = (
...
class SWDetect(Detect):
    ...
    def __init__(self, nc: int = 80, ch: Tuple = ()):
        ...
        self.cv3 = (
            nn.ModuleList(nn.Sequential(Conv(x, c3, 3), Conv(c3, c3, 3), nn.Conv2d(c3, self.nc, 1)) for x in ch)
            if self.legacy
            else nn.ModuleList(nn.Sequential(SWBlock(x, c3), nn.Conv2d(c3, self.nc, 1)) for x in ch)
        )
